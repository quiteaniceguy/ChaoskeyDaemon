...
#include "ex_NistStatisticalTestSuite.h"
#include <unistd.h>
...
char filePath[]="/home/teblen/workspace/NistTest/sts-2.1.2";


JNIEXPORT jstring JNICALL Java_ex_NistStatisticalTestSuite_runNistTests(JNIEnv *env, jclass cls){
	int		i;
	int		option;			/* TEMPLATE LENGTH/STREAM LENGTH/GENERATOR*/
	char	*streamFile;	/* STREAM FILENAME     */
	char *argv[]={"./assess","100000"};
	int argc=2;
	...
	return (*env)->NewStringUTF(env, "succesful");
}
...
{ 
	...
	sprintf(resultsDir, "%s/experiments/%s/%s/results.txt",filePath, generatorDir[option], testNames[testNameID]);
	...
	for ( i=0; i<numOfFiles; i++ ) {
		if ( i < 10 )
			sprintf(s[i], "%s/experiments/%s/%s/data%1d.txt",filePath, generatorDir[option], testNames[testNameID], i+1);
		else if (i < 100)
			sprintf(s[i], "%s/experiments/%s/%s/data%2d.txt", filePath, generatorDir[option], testNames[testNameID], i+1);
		else
			sprintf(s[i], "%s/experiments/%s/%s/data%3d.txt", filePath, generatorDir[option], testNames[testNameID], i+1);
	}
	numread = 0;
	m = numOfFiles/20;
	if ( (numOfFiles%20) != 0 )
...
{
	int		i, k, randomExcursionSampleSize = 0, generalSampleSize = 0;
		...
		if ( testVector[i] ) {
			...
			if ( ((i == TEST_CUSUM) && testVector[TEST_CUSUM] ) ||
				...
				for ( k=0; k<numOfFiles; k++ ) {
					if ( k < 10 )
						sprintf(s, "%s/experiments/%s/%s/data%1d.txt", filePath, generatorDir[option], testNames[i], k+1);
					else if ( k < 100 )
						sprintf(s, "%s/experiments/%s/%s/data%2d.txt", filePath, generatorDir[option], testNames[i], k+1);
					else
						sprintf(s, "%s/experiments/%s/%s/data%3d.txt", filePath, generatorDir[option], testNames[i], k+1);
				...
				sprintf(s, "%s/experiments/%s/%s/results.txt", filePath, generatorDir[option], testNames[i]);
				generalSampleSize = computeMetrics(s,i);
			}
		}
	}
