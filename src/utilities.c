...
openOutputStreams(int option)
{
	...
	sprintf(freqfn, "%s/experiments/%s/freq.txt",filePath, generatorDir[option]);
	if ( (freqfp = fopen(freqfn, "w")) == NULL ) {
		printf("\t\tMAIN:  Could not open freq file: <%s>", freqfn);
		exit(-1);
	}
	sprintf(summaryfn, "%s/experiments/%s/finalAnalysisReport.txt",filePath, generatorDir[option]);
	...
	for( i=1; i<=NUMOFTESTS; i++ ) {
		if ( testVector[i] == 1 ) {
			sprintf(statsDir, "%s/experiments/%s/%s/stats.txt", filePath, generatorDir[option], testNames[i]);
			sprintf(resultsDir, "%s/experiments/%s/%s/results.txt", filePath,generatorDir[option], testNames[i]);
